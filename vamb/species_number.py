"""!
 ...
 class SpeciesNumber:
     ...
     def _process_by_python(self):
         ...
         self.nclusters = len(self.definitive_clusters)
         return self
     ...
     def _statistical_optimization(self):
         ...
         for index in range(len(self.__clusters)):
             ...
             if not split:  #the cluster is not split!
                 ...
                 indices_to_keep = numpy.setdiff1d(np.arange(len(self.__data)), numpy.where(numpy.isin(self.__data, points_to_remove).all(axis=1)))
