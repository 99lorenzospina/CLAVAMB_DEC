...
class Composition:
    ...
    * matrix: The composition matrix itself
    ...
    @classmethod
    def read_contigs_augmentation(cls: type[C], filehandle, minlength=100, k=4, index_list = None, store_dir="./", backup_iteration=18, augmode=[-1,-1], use_pc = False):
            ...
            # AllAugmentation','GaussianNoise','Transition','Transversion','Mutation'
                ...
                for j2 in range(trans_count[i]):
                    ...
                    if not use_pc:
                        ...
                        _np.savez(f"{store_dir+_os.sep}pool{i}_k{k}_index{index_list[index]}_Transition_{j2}.npz", trans_save)
                    index += 1
                ...
                for j2 in range(traver_count[i]):
                    ...
                    if not use_pc:
                        ...
                        _np.savez(f"{store_dir+_os.sep}pool{i}_k{k}_index{index_list[index]}_Transversion_{j2}.npz", traver_save)
                    index += 1
                ...
                for j2 in range(mutated_count[i]):
                    ...
                    if not use_pc:
                        ...
                        _np.savez(f"{store_dir+_os.sep}pool{i}_k{k}_index{index_list[index]}_Mutation_{j2}.npz", mutated_save)
                    index += 1
    ...
    @classmethod
    def create_augmentation(cls: type[C], fastapaths, minlength=100, k=4, store_dir="./", backup_iteration=18, augmode=[-1,-1], use_pc = False):
        """Parses a FASTA file open in binary reading mode.

        Input:
            fastapaths: Paths of stuff to extend
            minlength: Ignore any references shorter than N bases [100]
            backup_iteration: numbers of generation for training
            store_dir: the dir to store the augmentation data
            augmode: the augmentation method. 0 for gaussian noise, 1 for transition, 2 for transversion, 3 for mutation, -1 for all.
